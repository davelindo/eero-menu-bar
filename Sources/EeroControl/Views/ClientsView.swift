...
struct ClientsView: View {
        ...
        ScrollView {
          VStack(alignment: .leading, spacing: 6) {
            if !primaryClients.isEmpty {
              sectionHeader(
                "Primary LAN · \(presentation.primaryOnlineCount)/\(primaryClients.count) online")
    ...
    Button {
      ...
      HStack(spacing: 10) {
        ...
        VStack(alignment: .leading, spacing: 2) {
          ...
          Text(
            [client.ip, client.mac, client.manufacturer, sourceLocationText(for: client)].compactMap
            { $0 }.joined(separator: " · ")
          )
          .font(.caption)
          .foregroundStyle(.secondary)
      ...
      .liquidGlass(
        ...
        tint: selectedClientID == client.id ? Color.blue.opacity(0.45) : .clear
      )
    }
    .buttonStyle(.plain)
  }
  ...
  private func clientDetailView(network: EeroNetwork, client: EeroClient) -> some View {
    VStack(alignment: .leading, spacing: 12) {
      ...
      KeyValueRow(label: "IP Address", value: client.ip ?? "Unavailable")
      KeyValueRow(label: "MAC Address", value: client.mac ?? "Unavailable")
      KeyValueRow(label: "Manufacturer", value: client.manufacturer ?? "Unavailable")
      KeyValueRow(label: "Device Type", value: client.deviceType ?? "Unavailable")
      KeyValueRow(label: "Connection", value: client.connectionType ?? "Unavailable")
      KeyValueRow(
        label: "Connected To eero", value: sourceLocationText(for: client) ?? "Unavailable")
      KeyValueRow(label: "Signal", value: signalText(for: client))
      KeyValueRow(label: "Channel", value: client.channel.map(String.init) ?? "Unavailable")
      ...
      KeyValueRow(label: "Last Active", value: client.lastActive ?? "Unavailable")
  ...
  private func filterChip(title: String, enabled: Bool, action: @escaping () -> Void) -> some View {
    Button(action: action) {
      Text(title)
        ...
        .liquidGlass(
          in: Capsule(),
          tint: enabled ? Color.blue.opacity(0.35) : .clear,
          interactive: true
        )
    }
    .buttonStyle(.plain)
  ...
  private func frequencyText(for client: EeroClient) -> String {
    ...
    return joined.isEmpty ? "Unavailable" : joined
  }
  ...
  private func rateText(for client: EeroClient) -> String {
    switch (client.rxRateMbps, client.txRateMbps) {
    case (nil, nil):
      return "Not reported"
    case (let rx?, let tx?) where abs(rx - tx) < 0.001:
      return "Link \(formatMbpsAdaptive(rx))"
    case (let rx?, let tx?):
      return "RX \(formatMbpsAdaptive(rx)) · TX \(formatMbpsAdaptive(tx))"
    case (let rx?, nil):
      return "RX \(formatMbpsAdaptive(rx))"
    case (nil, let tx?):
      return "TX \(formatMbpsAdaptive(tx))"
    }
  }

  private func usageText(for client: EeroClient) -> String {
    if client.usageDownMbps != nil || client.usageUpMbps != nil {
      let down = client.usageDownMbps.map(formatMbpsAdaptive) ?? "--"
      let up = client.usageUpMbps.map(formatMbpsAdaptive) ?? "--"
      return "↓\(down) ↑\(up)"
    }

    if client.usageDownPercentCurrent != nil || client.usageUpPercentCurrent != nil {
      let downPercent = client.usageDownPercentCurrent.map { "\($0)%" } ?? "--"
      let upPercent = client.usageUpPercentCurrent.map { "\($0)%" } ?? "--"
      return "↓\(downPercent) ↑\(upPercent) current load"
    }

    return "Not reported (live telemetry not provided)"
  }

  private func formatMbpsAdaptive(_ value: Double) -> String {
    let sanitized = max(0, value)
    if sanitized >= 1_000 {
      return String(format: "%.2f Gbps", sanitized / 1_000)
    }
    if sanitized >= 100 {
      return String(format: "%.0f Mbps", sanitized)
    }
    if sanitized >= 1 {
      return String(format: "%.1f Mbps", sanitized)
    }

    let kbps = sanitized * 1_000
    if kbps >= 1 {
      return String(format: "%.1f Kbps", kbps)
    }

    let bps = sanitized * 1_000_000
