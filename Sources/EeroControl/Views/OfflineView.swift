...
struct OfflineView: View {
    ...
    private var localProbeCard: some View {
        SectionCard(title: "Local Probes") {
            HStack(spacing: 8) {
                StatusChip(icon: "network", text: appState.offlineSnapshot.localHealthLabel, tone: toneForLanStatus())
                StatusChip(icon: "list.bullet.clipboard", text: "Pending \(pendingActionCount)", tone: pendingActionCount > 0 ? .warning : .success)
                if failedActionCount > 0 {
                    StatusChip(icon: "exclamationmark.triangle.fill", text: "Failed \(failedActionCount)", tone: .danger)
                }
            }
