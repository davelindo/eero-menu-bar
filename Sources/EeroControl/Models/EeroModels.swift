...
struct EeroAccountSnapshot: Codable, Equatable, Sendable {
  ...
  var modelAudit: ModelFieldAuditSummary? = nil
...
struct ModelFieldAuditSummary: Codable, Equatable, Sendable {
  var generatedAt: Date
  var networkFields: [String: ModelFieldAuditCounter]
  var clientFields: [String: ModelFieldAuditCounter]
  var deviceFields: [String: ModelFieldAuditCounter]
}

struct ModelFieldAuditCounter: Codable, Equatable, Sendable {
  var present: Int
  var total: Int
}
...
struct EeroClient: Codable, Equatable, Identifiable, Sendable {
  ...
  var usageDownPercentCurrent: Int?
  var usageUpPercentCurrent: Int?
...
struct EeroProfile: Codable, Equatable, Identifiable, Sendable {
  ...
  var availableApplications: [EeroBlockedApplication]?
  var filters: ProfileFilterState
  var resources: [String: String]
}

struct EeroBlockedApplication: Codable, Equatable, Sendable, Identifiable {
  var id: String
...
extension EeroEthernetPortStatus {
  var peerGroupingKey: String {
    if let interfaceNumber {
      return "if-\(interfaceNumber)"
    }
    if let portName {
      return "port-\(portName.trimmingCharacters(in: .whitespacesAndNewlines).lowercased())"
    }
    return "id-\(id)"
  }

  func peerCountDescription(allPortStatuses: [EeroEthernetPortStatus]) -> String {
    let peers = resolvedPeerCount(allPortStatuses: allPortStatuses)
    if peers == 0 {
      return "No peers"
    }
    if peers == 1 {
      return resolvedPeerName(allPortStatuses: allPortStatuses)
        ?? "1 peer"
    }
    return peers == 1 ? "1 peer" : "\(peers) peers"
  }

  func resolvedPeerName(allPortStatuses: [EeroEthernetPortStatus]) -> String? {
    let allNames =
      allPortStatuses
      .filter { $0.peerGroupingKey == peerGroupingKey }
      .map(\.neighborName)
      .compactMap { $0?.trimmingCharacters(in: .whitespacesAndNewlines) }
      .filter { !$0.isEmpty }

    if let local = neighborName?.trimmingCharacters(in: .whitespacesAndNewlines), !local.isEmpty {
      return local
    }
    if let peerURL = neighborURL {
      let fromURL = DictionaryValue.id(fromURL: peerURL)
      if !fromURL.isEmpty {
        return fromURL
      }
    }
    return allNames.first
  }

  func resolvedPeerCount(allPortStatuses: [EeroEthernetPortStatus]) -> Int {
    if let explicitCount = peerCount {
      return max(0, explicitCount)
    }

    let siblingCount = allPortStatuses.filter { $0.peerGroupingKey == peerGroupingKey }.count
    if siblingCount > 0 {
      return siblingCount
    }

    if (hasCarrier == true) || (neighborName != nil) || (neighborURL != nil) {
      return 1
    }
