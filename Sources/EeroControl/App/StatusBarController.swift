...
final class StatusBarController: NSObject, NSPopoverDelegate {
  ...
  init(appState: AppState) {
    ...
    if let button = statusItem.button {
      ...
      LiquidGlassStyleUtility.configureStatusViewGlass(button)
    }
  ...
  private func applyPopoverGlassStyle() {
    guard let window = popover.contentViewController?.view.window else { return }
    LiquidGlassStyleUtility.stylePopoverWindow(window)
  }
}

private enum LiquidGlassStyleUtility {
  static func configureStatusViewGlass(_ button: NSStatusBarButton) {
    button.contentTintColor = NSColor.labelColor
    button.wantsLayer = true
    button.layer?.backgroundColor = NSColor.controlAccentColor.withAlphaComponent(0.05).cgColor
    ...
    if container.subviews.first(where: { $0 is StatusItemGlassEffectView }) == nil {
      let effectView = StatusItemGlassEffectView()
      effectView.translatesAutoresizingMaskIntoConstraints = false
      effectView.configureForStatusGlass()
      container.addSubview(effectView, positioned: .below, relativeTo: button)
      NSLayoutConstraint.activate([
    ...
    container
      ...
      .forEach { $0.configureForStatusGlass() }
  }

  static func stylePopoverWindow(_ window: NSWindow) {
    window.isOpaque = false
    let popoverBackground = NSColor.windowBackgroundColor.withAlphaComponent(0.88).cgColor
    window.backgroundColor = NSColor.windowBackgroundColor.withAlphaComponent(0.88)
    window.titlebarAppearsTransparent = true
    window.styleMask.insert(.fullSizeContentView)

    window.contentView?.wantsLayer = true
    window.contentView?.layer?.isOpaque = false
    window.contentView?.layer?.backgroundColor = popoverBackground

    window.contentView?.superview?.wantsLayer = true
    window.contentView?.superview?.layer?.isOpaque = false
    window.contentView?.superview?.layer?.backgroundColor = popoverBackground

    guard let contentView = window.contentView else { return }

    if contentView.subviews.first(where: { $0 is StatusPopoverGlassEffectView }) == nil {
      let effectView = StatusPopoverGlassEffectView()
      effectView.translatesAutoresizingMaskIntoConstraints = false
...
private final class StatusPopoverGlassEffectView: NSVisualEffectView {}
private final class StatusItemGlassEffectView: NSVisualEffectView {}

extension NSVisualEffectView {
  fileprivate func configureForStatusGlass() {
    material = .titlebar
